...
public class youtubeplaylist {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		for (int i = 0; i < songs.length; i++) {
			...
			// skipping different kinds of ads
			try {
				driver.manage().timeouts().implicitlyWait(3, TimeUnit.SECONDS);
				String TypeOfAd = driver.findElement(By.xpath("//span[@class='ytp-ad-simple-ad-badge']//div[@class='ytp-ad-text']")).getText();
				System.out.println(TypeOfAd);

				if (TypeOfAd.equals(com1)) {
					String addDurtion = driver.findElement(By.className("ytp-time-duration")).getText();
					System.out.println(addDurtion);
					skipAd(addDurtion);
			...
			// time duration from string to double
			Thread.sleep(500);
			...
			double durtime = Double.parseDouble(time1);
			 ...
			 //taking the  real-time duration from the video song skipping 4 sec before the video ends 
			while (durtime == durtime) {
				...
				act.moveToElement(fullScreenButton).perform();
